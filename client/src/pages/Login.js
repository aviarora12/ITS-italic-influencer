import React from 'react';

export default function Login() {
  const handleGoogleLogin = () => {
    window.location.href = '/auth/google';
  };

  return (
    <div className="min-h-screen bg-[#0f1117] flex items-center justify-center p-4">
      <div className="max-w-md w-full">
        <div className="text-center mb-10">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-indigo-600 rounded-2xl text-3xl font-bold mb-6">I</div>
          <h1 className="text-3xl font-bold text-white mb-2">Italic Influencer Hub</h1>
          <p className="text-gray-400">Manage your influencer campaigns end-to-end</p>
        </div>

        <div className="bg-[#1a1d27] border border-[#2d3148] rounded-2xl p-8">
          <h2 className="text-lg font-semibold text-white mb-2">Sign in to get started</h2>
          <p className="text-sm text-gray-400 mb-6">
            Connect your Google account to access your campaigns and sync with Google Sheets.
          </p>

          <button
            onClick={handleGoogleLogin}
            className="w-full flex items-center justify-center gap-3 bg-white text-gray-800 font-semibold py-3 px-6 rounded-xl hover:bg-gray-100 transition-colors"
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            Continue with Google
          </button>

          <div className="mt-6 pt-6 border-t border-[#2d3148]">
            <p className="text-xs text-gray-500 text-center">
              Google Sheets API access is required to store and sync your campaign data.
              Your data stays in your own Google account.
            </p>
          </div>
        </div>

        <div className="mt-6 grid grid-cols-3 gap-4 text-center">
          {[
            { icon: '◈', label: 'Campaign tracking' },
            { icon: '◉', label: 'Influencer CRM' },
            { icon: '◎', label: 'Smart reminders' },
          ].map(f => (
            <div key={f.label} className="bg-[#1a1d27]/50 border border-[#2d3148] rounded-xl p-3">
              <div className="text-2xl mb-1">{f.icon}</div>
              <div className="text-xs text-gray-400">{f.label}</div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
}
